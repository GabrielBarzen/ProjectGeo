...
class CreateGraph extends GraphRenderer implements Graph {
	...
	public get vertices(): Vertex[] {
		return this.vertexMarkers.map((marker) => {
			return marker.toVertex();
		});
	}
	...
	splitLine(sourceVertex: Vertex, destinationVertex: Vertex, position: L.LatLng) {
		...
		if (
			Math.max(sourceId, destinationId) == this.vertexMarkers.length - 1 &&
			Math.min(sourceId, destinationId) != this.vertexMarkers.length - 2
		) {
			this.addVertex(position.lat, position.lng, '0');
		} else {
			this.addVertex(position.lat, position.lng, String(Math.max(sourceId, destinationId)));
		}
	}
	...
	renderTo(map: L.Map): void {
		...
		this.vertexMarkers.forEach((vertexMarker) => {
			...
			vertexMarker.setClickDragBehaviour((vertexMarker) => {
	...
	private resolveConnectionsByListOrder() {
		this.vertexMarkers.forEach((vertex) => {
			...
			if (vertex.id == '0') {
				...
				return;
			} else if (vertex.id == String(this.vertexMarkers.length - 1)) {
				vertex.connections.push(String(Number(vertex.id) - 1));
